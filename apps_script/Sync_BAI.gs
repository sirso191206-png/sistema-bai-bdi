/**
 * SINCRONIZACIÓN AUTOMÁTICA: Google Forms (BAI) -> Supabase   [v2 corregido]
 * --------------------------------------------------------------------------
 ...
 *  - Activador: alEnviarFormularioBAI / "De la hoja de cálculo" /
 ...
 */
function alEnviarFormularioBAI(e) {
  try {
    ...
    const pacienteId = obtenerOCrearPaciente(nombreCompleto, null, fechaNacimiento ? normalizarFecha(fechaNacimiento) : null, curp);

    const fechaISO = USAR_MARCA_TEMPORAL
      ? Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd')
      : normalizarFecha(fechaAplicacion);

    const evaluacionId = obtenerOCrearEvaluacion(pacienteId, fechaISO);
    guardarRespuestasBAI(evaluacionId, respuestasNumericas);

    Logger.log('BAI sincronizado correctamente para: ' + nombreCompleto + ' (' + fechaISO + ')');
  } catch (error) {
    Logger.log('ERROR sincronizando BAI: ' + error.message);
    // Opcional: MailApp.sendEmail('oiztex@gmail.com', 'Error sincronización BAI', error.message);
  }
}
...
function obtenerOCrearEvaluacion(pacienteId, fechaISO) {
  const urlBusqueda = SUPABASE_URL + '/rest/v1/evaluaciones?paciente_id=eq.' + pacienteId +
    '&fecha=eq.' + fechaISO + '&select=id';
  ...
  const payload = { paciente_id: pacienteId, fecha: fechaISO };
...
function guardarRespuestasBAI(evaluacionId, respuestas) {
  const payload = { evaluacion_id: evaluacionId };
  for (let i = 0; i < 21; i++) {
    payload['respuesta_' + (i + 1)] = respuestas[i] !== undefined ? respuestas[i] : 0;
  }

  const url = SUPABASE_URL + '/rest/v1/bai?on_conflict=evaluacion_id';
  const resp = UrlFetchApp.fetch(url, {
    method: 'post',
...
function probarConUltimaFila() {
  ...
  alEnviarFormularioBAI(eventoSimulado);
}
